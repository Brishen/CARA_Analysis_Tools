...
Expected values from the MATLAB example:
...
def run_example1() -> tuple[float, float]:
    """Run the Hall example 1 and return ``(Pc2D, Pc3D)``."""

    r1 = np.array([-9.841950433215101e05, 3.932342044549424e05, 6.991223682230414e06])
    v1 = np.array([4.883696742000000e03, 5.689086045000000e03, 3.665361590000000e02])
    cov1 = np.array(
        [
    ...
    r2 = np.array([-9.839696058965517e05, 3.936845951174244e05, 6.991219291625473e06])
    v2 = np.array([1.509562687000000e03, 7.372938617000000e03, -1.492509430000000e02])
    cov2 = np.array(
        [
    ...
    hbr = 20.0
    pc2d = np.real_if_close(PcElrod(
        r1.reshape(1, 3), v1.reshape(1, 3), cov1.reshape(1, 6, 6),
        r2.reshape(1, 3), v2.reshape(1, 3), cov2.reshape(1, 6, 6), hbr
    )[0][0]).item()
    pc3d = np.real_if_close(Pc3D_Hall(r1, v1, cov1, r2, v2, cov2, hbr)[0]).item()

    return pc2d, pc3d


def main() -> None:
    """Run and print Pc results in the same format as the MATLAB example."""

    pc2d, pc3d = run_example1()
    print(f"Pc2D = {pc2d:0.7e}")
